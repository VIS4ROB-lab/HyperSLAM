...
using PixelEvaluatorTestsTypes = ::testing::Types<SE3<Scalar>>;

TYPED_TEST_SUITE_P(PixelEvaluatorTests);

template <typename TAmbientSpace>
class PixelEvaluatorTests
 ...
 public:
  ...
  using Observation = VisualPixelObservation;
  using Landmark = Observation::Landmark;
  using Metric = CartesianMetric<Pixel<Scalar>>;
  ...
  static auto checkGradients() -> bool {
    ...
    const auto pixel = Pixel<Scalar>::Random().eval();
    auto measurement = Measurement{stamp, *camera, pixel};
...
TYPED_TEST_P(PixelEvaluatorTests, Gradients) {
  EXPECT_TRUE(this->checkGradients());
}

REGISTER_TYPED_TEST_SUITE_P(PixelEvaluatorTests, Gradients);

INSTANTIATE_TYPED_TEST_SUITE_P(HyperSystemTests, PixelEvaluatorTests, PixelEvaluatorTestsTypes);
