...
auto InertialEvaluator<SE3<Scalar>>::evaluate(const Query& query, DynamicJacobian<Scalar>* J_e) const -> Result {
  ...
  const auto num_gyroscope_bias_parameters = imu.gyroscopeBias().interpolator()->layout().outer.size() + 1;
  const auto o_T_bs = layout.static_sensor_idx + Traits<IMU>::kTransformationOffset;
  const auto o_i_g = layout.static_sensor_idx + Traits<IMU>::kGyroscopeIntrinsicsOffset;
  const auto o_i_a = layout.static_sensor_idx + Traits<IMU>::kAccelerometerIntrinsicsOffset;
  const auto o_S_g = layout.static_sensor_idx + Traits<IMU>::kGyroscopeSensitivityOffset;
  const auto o_X_a = layout.static_sensor_idx + Traits<IMU>::kAccelerometerAxesOffsetsOffset;
  const auto o_b_g = layout.dynamic_sensor_idx;
  const auto o_b_a = o_b_g + num_gyroscope_bias_parameters;
  const auto o_g_w = layout.static_observation_idx;
  ...
  if (!J_e) {
    ...
    const auto S_wb = state.evaluate(q_S_wb, p_ps + layout.static_state_idx);
    ...
    if (J_S_wb) {
      ...
      J_e->middleCols(offsets[layout.static_state_idx], S_wb.jacobians[kValueIndex].cols()).noalias() = J_value * S_wb.jacobians[kValueIndex] + J_velocity * S_wb.jacobians[kVelocityIndex] + J_acceleration * S_wb.jacobians[kAccelerationIndex];
    }
    ...
    if (p_js[o_T_bs]) {
      ...
      J_e->middleCols(offsets[o_T_bs], sizes[o_T_bs]).noalias() = J_T_bs * SE3JacobianAdapter(T_bs.data());
    }

    if (p_js[o_i_g]) {
      Jacobian<Tangent::Angular, GyroscopeIntrinsics> J_i_g;
      i_g.align(w_s, nullptr, J_i_g.data());
      J_e->block(Traits::kAngularOffset, offsets[o_i_g], Traits::kNumAngularParameters, sizes[o_i_g]).noalias() = J_i_g;
    }

    if (p_js[o_i_a]) {
      Jacobian<Tangent::Linear, AccelerometerIntrinsics> J_i_a;
      i_a.align(a_s, nullptr, J_i_a.data());
      J_e->block(Traits::kLinearOffset, offsets[o_i_a], Traits::kNumLinearParameters, sizes[o_i_a]).noalias() = J_i_a;
    }

    if (p_js[o_S_g]) {
      auto J_S_g = J_e->block(Traits::kAngularOffset, offsets[o_S_g], Traits::kNumAngularParameters, sizes[o_S_g]);
      J_S_g(0, 0) = a_b_m[0];
      J_S_g(1, 1) = a_b_m[0];
      J_S_g(2, 2) = a_b_m[0];
      J_S_g(0, 3) = a_b_m[1];
    ...
    if (p_js[o_X_a]) {
      auto J_X_a = J_e->block(Traits::kLinearOffset, offsets[o_X_a], Traits::kNumLinearParameters, sizes[o_X_a]);
    ...
    if (J_b_g) J_e->block(Traits::kAngularOffset, offsets[o_b_g], Traits::kNumAngularParameters, b_g.jacobians[kValueIndex].cols()).noalias() = b_g.jacobians[kValueIndex];
    if (J_b_a) J_e->block(Traits::kLinearOffset, offsets[o_b_a], Traits::kNumLinearParameters, b_a.jacobians[kValueIndex].cols()).noalias() = b_a.jacobians[kValueIndex];
    if (p_js[o_g_w]) J_e->block(Traits::kLinearOffset, offsets[o_g_w], Traits::kNumLinearParameters, sizes[o_g_w]).noalias() = Scalar{-1} * I_a_R_sb * R_bw;
