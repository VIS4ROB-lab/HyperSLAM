...
auto Manifold<IMU, OptimizerSuite::CERES>::manifolds(const hyper::Stamp& /* stamp */) const -> Pointers<ceres::Manifold> {
  const auto num_gyroscope_bias_manifolds = sensor().gyroscopeBias().interpolator()->layout().outer.size() + 1;
  const auto num_accelerometer_bias_manifolds = sensor().accelerometerBias().interpolator()->layout().outer.size() + 1;
  return assembleManifolds(num_gyroscope_bias_manifolds, num_accelerometer_bias_manifolds);
}
